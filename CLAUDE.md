...
**Multicast bind address (`config.ts` `multicastBindAddress`, default `0.0.0.0`)**: deliberately NOT `127.0.0.1` by default, even though that reads as the more secure loopback-only choice. UDP multicast group membership is interface-scoped on *both* ends of this handshake — this server's client socket AND the UE editor's own independent Multicast Bind Address project setting (`RemoteExecutionMulticastBindAddress` in `PythonScriptPluginSettings`, engine default `127.0.0.1` per `PythonScriptPluginSettings.cpp`, but commonly overridden to `0.0.0.0` on real multi-NIC hosts — confirmed true of this repo's own test project's `DefaultEngine.ini`). If the editor's own setting is `0.0.0.0`, its discovery reply leaves over a real NIC regardless of what this server's own bind address is set to, so narrowing only the client side cannot fix multi-NIC discovery failures — confirmed empirically (0 nodes found narrowing client-only, even with `resolveMulticastInterface()`'s outbound interface also forced to loopback to match). `0.0.0.0` is the one setting that works without requiring the editor's own setting to be known/matched first. `UNREAL_MCP_MULTICAST_BIND`/`--multicast-bind` remain available for hosts where both sides have been verified to work on loopback. Not related to Epic's official MCP server running alongside this one — that uses a separate in-process HTTP listener (port 8000) and never touches this transport. Full writeup: README's security note under Unreal Editor Setup.
